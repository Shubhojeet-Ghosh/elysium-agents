...
def add_visitor_to_agent(agent_id, chat_session_id, sid=None, geo_data=None, visitor_at=None, alias_name=None):
    ...
    Replaces any other online socket for the same chat_session_id so one session
    maps to at most one live connection (avoids double count on reconnect).
    ...
    try:
        ...
        for existing_sid, data in visitors.items():
            if existing_sid == sid:
                continue
            try:
                ...
                continue
            if existing.get("chat_session_id") == chat_session_id:
                client.hdel(key, existing_sid)
                logger.info(
                    f"Removed stale visitor sid {existing_sid} for chat_session_id "
                    f"{chat_session_id} on agent {agent_id} (replaced by {sid})"
                )
...
def remove_visitor_from_agent(agent_id, sid) -> bool | None:
    """
    Remove a visitor from the agent's visitors hash in Redis.

    Returns:
        True if the sid was present and removed, False if not found, None on error.
    """
    try:
        client = get_redis_client()
        key = f"atlas_{agent_id}_visitors"
        ...
        if result:
            ...
            logger.info(
                f"Visitor with sid {sid} was not found in agent {agent_id} visitors hash "
                f"(already removed or replaced)"
            )
...
def get_online_visitors_map_by_chat_session(agent_id: str) -> dict[str, dict]:
    """
    Map chat_session_id -> live visitor payload for all online visitors (Redis hash).

    Each value includes sid and in_conversation_with when the visitor is connected.
    """
    try:
        client = get_redis_client()
        key = f"atlas_{agent_id}_visitors"
        ...
        for sid, data in visitors.items():
            ...
            if not visitor.get("sid"):
                visitor["sid"] = sid
            result[chat_session_id] = visitor
        return result
    except Exception as e:
        logger.error(f"Error building online visitors map for agent {agent_id}: {e}")
        return {}
