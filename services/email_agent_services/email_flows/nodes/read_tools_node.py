...
def _resolve_tool_ids(agent: Dict[str, Any], config: Dict[str, Any]) -> List[str]:
    config_ids = config.get("tool_ids") or []
    if config_ids:
        return [str(tool_id).strip() for tool_id in config_ids if str(tool_id).strip()]

    agent_ids = agent.get("tool_ids") or []
    return [str(tool_id).strip() for tool_id in agent_ids if str(tool_id).strip()]
...
def _set_tool_context_defaults(context: Dict[str, Any]) -> None:
    context["registered_tools"] = []
    context["tools_planned"] = []
...
async def execute_read_tools_node(
    ...
    """
    LLM tool planning + HTTP execution for agent-registered external tools.

    Writes context.registered_tools, context.tools_planned, context.tool_results.
    Full tool API responses are stored in tool_results[].response for generate_email.
    """
    started_at = _utc_now()
    tool_ids = _resolve_tool_ids(agent, config)
    ...
    logger.info(
        f"read_tools_node started thread_id={thread_id} tool_ids={tool_ids} "
        f"llm_model={llm_model}"
    )

    try:
        if not tool_ids:
            _set_tool_context_defaults(context)
            logger.info(
                f"read_tools_node skipped thread_id={thread_id} — no tool_ids configured"
            )
            completed_at = _utc_now()
            node_log = {
                "node_id": NODE_ID,
                "node_type": NODE_TYPE_READ_TOOLS,
        ...
        if not active_tools:
            _set_tool_context_defaults(context)
            logger.warning(
                f"read_tools_node skipped thread_id={thread_id} — "
                f"configured tool_ids={tool_ids} but no active tools resolved. "
            ...
            node_log = {
                ...
                "output": _build_skip_output(
                    context=context,
                    skip_reason="No active tools resolved for configured tool_ids.",
                    tools_registered=True,
                    configured_tool_ids=tool_ids,
                    registered_tools=[],
        ...
        logger.info(
            f"read_tools_node loaded {len(active_tools)} active tool(s) "
            f"for thread_id={thread_id}: "
            f"{[tool['tool_name'] for tool in registered_tools]}"
        )

        result = await plan_and_execute_email_tools(
            context=context,
            tools=active_tools,
            llm_model=llm_model,
            max_tool_calls=max_tool_calls,
        ...
        context["registered_tools"] = registered_tools
        context["tools_planned"] = tools_planned
        context["tool_results"] = tool_results

        if llm_decision == "called":
            logger.info(
                f"read_tools_node finished thread_id={thread_id} — LLM called "
                f"{len(tool_executions)} tool(s)"
            )
            for execution in tool_executions:
                logger.info(
                    "read_tools_node tool execution: "
            ...
            logger.info(
                f"read_tools_node finished thread_id={thread_id} — "
            ...
            "output": {
                ...
                "downstream_hints": {
                    ...
                    "ai_department_router": {
                        "uses": ["thread", "kb_chunks", "tool_results"],
                    },
                },
            },
            "error": None,
        ...
        logger.error(
            f"read_tools_node failed thread_id={thread_id} tool_ids={tool_ids}: {exc}",
            exc_info=True,
        )
        completed_at = _utc_now()
        duration_ms = int((completed_at - started_at).total_seconds() * 1000)
